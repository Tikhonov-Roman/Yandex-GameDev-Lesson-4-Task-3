...
public class SpawnExample : MonoBehaviour
{
    ...
    private void OnTick()
    {
        ...
        if (chance < 40)
        {
            Nlo firstTeamNlo = new Nlo(null, GetRandomPositionOutsideScreen(), Config.NloSpeed);
            Nlo secondTeamNlo = new Nlo(firstTeamNlo, GetRandomPositionOutsideScreen(), Config.NloSpeed);
            firstTeamNlo.SetTarget(secondTeamNlo);
            _factory.CreateNlo(firstTeamNlo);
            _factory.CreateNlo(secondTeamNlo);
        }
        else
        {
